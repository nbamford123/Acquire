...
Deno.test('actionHandlers type safety tests', async (t) => {
    ...
    for (const [actionType, handler] of Object.entries(actionHandlers)) {
      ...
      switch (actionType) {
        case ActionTypes.ADD_PLAYER:
          action = { type: actionType, payload: { playerName: 'Test' } };
          break;
        case ActionTypes.REMOVE_PLAYER:
          action = { type: actionType, payload: { playerName: 'Test' } };
          break;
        case ActionTypes.START_GAME:
          action = { type: actionType, payload: { playerName: 'TestPlayer' } };
          break;
        case ActionTypes.PLAY_TILE:
          action = { type: actionType, payload: { playerId: 0, tile: { row: 0, col: 0 } } };
          break;
        case ActionTypes.BUY_SHARES:
          action = { type: actionType, payload: { playerId: 0, shares: {} } };
          break;
        case ActionTypes.FOUND_HOTEL:
          action = { type: actionType, payload: { playerId: 0, hotelName: 'Worldwide' as const } };
          break;
        case ActionTypes.RESOLVE_MERGER:
          action = { type: actionType, payload: { playerId: 0, shares: { sell: 0, trade: 0 } } };
          break;
  ...
  await t.step('all handlers return GameState objects', () => {
    ...
    assertEquals(Array.isArray(result.hotels), true);
    assertEquals(Array.isArray(result.tiles), true);
    assertEquals(Array.isArray(result.lastActions), true);
  });
});
