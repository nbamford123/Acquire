...
import { GameError, GameErrorCodes, GamePhase, type GameState, type Player } from "@/engine/types/index.ts";
import { ActionTypes } from "@/engine/types/actionsTypes.ts";
import { INITIAL_PLAYER_MONEY, MINIMUM_PLAYERS, ROWS, COLS } from "@/engine/config/gameConfig.ts";
...
Deno.test("startGameReducer successful start tests", async (t) => {
  ...
  await t.step("successfully starts game with multiple players and sorts them", () => {
    ...
    for (let i = 0; i < firstTiles.length - 1; i++) {
      ...
      const isOrdered = tile1.col < tile2.col || (tile1.col === tile2.col && tile1.row <= tile2.row);
